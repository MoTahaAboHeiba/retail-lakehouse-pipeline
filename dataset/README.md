# Agentic DB Data Engineering Project
...
- `walmart_dataset/load_data.py`: Dataset loading logic for the Walmart CSV files.
- `walmart_dataset/ddl/walmart_schema.sql`: A schema definition that models Walmart business entities such as customers, employees, orders, products, stores, and order items.
- `walmart_dataset/data/*.csv`: Source data files for the Walmart dataset.
...
## How to Use
1. Review the schema in `walmart_dataset/ddl/walmart_schema.sql` to understand the data model.
2. Place the source CSV files in `walmart_dataset/data/`.
3. Run `python import_csv.py` to load the data using the project pipeline.
