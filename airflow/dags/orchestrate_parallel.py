...
def orchestrate_parallel():
    ...
    def check_s3_pipeline_health():
        ...
        This is the failure-visibility check."""
      
        conn = BaseHook.get_connection("databricks_default")
        ws = WorkspaceClient(host=conn.host, token=conn.password)

        pipeline_id = os.getenv("DATABRICKS_S3_PIPELINE_ID")
    ...
    silver_business_t = silver_business()
    silver_business_tests_t = silver_business_tests()
    ...
    # Fork: Sales branch
    dim_date_t >> silver_business_t >> silver_business_tests_t >> gold_eph_t >> gold_snap_t >> fact_orders_t
